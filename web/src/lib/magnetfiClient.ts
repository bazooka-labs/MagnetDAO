...
const SPEC = { psm: "/contracts/PSM.arc56.json", vault: "/contracts/Vault.arc56.json" } as const;
...
export async function openVault(al: AlgorandClient, sender: string, lpDisplay: number, borrowDisplay: number) {
  ...
  await al.newGroup()
    ...
    .addAppCallMethodCall(await vc.params.call({ method: "open_vault", args: [BigInt(ACTIVE.poolId), toBase(borrowDisplay)], maxFee: MAX_FEE }))
